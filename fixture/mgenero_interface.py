...
def create_all(template, config, params):
    # TODO:
    directory = os.path.abspath(config['build_folder'])
    ...
    generate_text = get_generate_text(config['template_name'], directory)
...
def get_generate_text(template_name, directory):
    text = f''' # autogenerated
from dave.mgenero.mgenero import ModelCreator
config = '{os.path.join(directory, 'circuit.cfg')}'
interface = '{os.path.join(directory, 'interface.yaml')}'
template = '{os.path.join(directory, template_name)}'
params = '{os.path.join(directory, 'params.yaml')}'
intermediate = '{os.path.join(directory, 'template.intermediate.sv')}'
output = '{os.path.join(directory,'final.sv')}'
